...
public class ProductService : IProductService {
  ...
  async Task<CommonResponse<List<ProductDTO>>>
      IProductService.getAllProducts() {
    CommonResponse<List<ProductDTO>> response =
        new CommonResponse<List<ProductDTO>>();
    try {
      ...
      if (products.Count > 0) {
        ...
        products.ForEach(async (Product product) => {
          ...
          productDTO.SubCategory =
              _mapper.Map<SubCategory, SubCategoryDTO>(subCategory!);

          Store? store = await _context.Stores.FirstOrDefaultAsync(
  ...
  CommonResponse<ProductDTO>
      IProductService.getProductByCategoryId(int categoryId) {
    throw new NotImplementedException();
  }
